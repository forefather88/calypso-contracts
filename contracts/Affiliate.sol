...
contract Affiliate {
    ...
    function increaseNumberAddress(uint256 _numberAddress) external {
        ...
        IERC20(oracle.getCalAddress()).transferFrom(msg.sender, address(this), _numberAddress.mul(1 ether));
        maxNumberOf[msg.sender] = maxNumberOf[msg.sender].add(_numberAddress);
    }
    
    function sendTokenAward(address[] memory _affiliates, uint256[] memory _awards, address _currency) external {
    ...
    function sendEthAward(address[] memory _affiliates, uint256[] memory _awards) external payable {
        uint256 totalAward = saveAward(_affiliates, _awards, address(0));
        require(totalAward == msg.value);
    }
    
    function saveAward(address[] memory _affiliates, uint256[] memory _awards, address _currency) internal returns (uint256 _total) {
        uint256 totalAward;
        for (uint256 i = 0; i < _affiliates.length; i++) {
            ...
            awardsOf[_affiliate][_currency] = awardsOf[_affiliate][_currency].add(_award);
        }
        return totalAward;
    }
    ...
    function unStake() external {
        ...
        IERC20(oracle.getCalAddress()).transfer(msg.sender, numberAddr.mul(1 ether));
    ...
    function saveMultiAddrs(address[] memory _addAddrs, address[] memory _removeAddrs) external {
    ...
    function getAffiliateStatus(address[] memory _currencies) external view returns (
        uint256 _maxNumber, 
    ...
    function addAddress(address _addr) public {
        ...
        require(referralsOf[msg.sender].length <= maxNumberOf[msg.sender], "Over your max referrals.");
    }
    ...
    function remove(address _affiliate, address _addr) internal {
        require(_affiliate == affiliateOf[_addr] && referralsOf[_affiliate].length > 0);
