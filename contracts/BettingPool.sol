...
contract BettingPool {
    ...
    mapping(bytes32 => uint256) private betIdToIndex;
    mapping(address => bytes32[]) private userBetIds;
    ...
    function addMaxCap(uint256 _depositedCal) public onlyOwner returns (bool) {
        ...
        IERC20(poolManager.getCalAddress()).transferFrom(msg.sender, address(this), _depositedCal);
        return true;
    }
    
    function getPoolDetail() external view returns 
            (string memory _title,
            string memory _description,
            uint256 _gameId,
            string memory _gameType,
    ...
    function getPoolDetail2() external view returns (
        uint8 _result,
        uint256 _total,
        uint256 _winOutcome,
        uint256 _winTotal,
    ...
    function betWithEth(uint8 _side) external payable onlyBeforeEndDate returns (bytes32) {
        require(currency == address(0), "The pool is using another currency");
        return placeBet(msg.sender, _side, msg.value);
    }
    
    function betWithToken(uint8 _side, uint256 _amount) external onlyBeforeEndDate returns (bytes32) {
        require(currency != address(0), "This pool is using ETH");
        require(IERC20(currency).transferFrom(msg.sender, address(this), _amount), "Can not deposit tokens");
        return placeBet(msg.sender, _side, _amount);
    }

    function placeBet(address _bettor, uint8 _side, uint256 _amount) internal returns (bytes32) {
        ...
        bytes32 id = keccak256(abi.encodePacked(_side, _amount, timestamp));
    ...
    function getBetIdsOf() external view returns (bytes32[] memory _betIds) {
        _betIds = userBetIds[msg.sender];
    }
    
    function getBet(bytes32 _betId) external view returns 
        (   address _bettor, 
            uint8 _side, 
            uint256 _amount, 
            uint256 _createdDate) {
    ...
    function setResult(uint8 _sideWin) external onlyEndDate returns (bool) {
        ...
        if (total > 0) {
            ...
            platformFeeAmount = total.mul(poolManager.getPlatformFee()).div(10000);
    ...
    function claimReward() external returns (bool) {
        ...
        uint256 amount;
        if (block.timestamp - endDate > WAIT_TIME_FOR_RESULT && result == 0) { // can withdraw after 5 hours 
    ...
    function withdraw(address payable receiver, uint256 amount) internal returns (bool) {
        require(amount > 0);
    ...
    function withdrawDepositAndFee() external onlyOwner returns (bool) {
        require(result != 0 || result == 0 && block.timestamp - endDate > WAIT_TIME_FOR_RESULT);
    ...
    function forwardAffiliateAward() internal {
        ...
        for (uint256 i = 0; i < betUsers.length; i++) {
            ...
            if (userBet[_addr] > 0 && _affiliate != address(0)) {
                ...
                uint256 award = userBet[_addr].mul(poolManager.getAffiliatePercent()).div(10000);
                awards.push(award);
