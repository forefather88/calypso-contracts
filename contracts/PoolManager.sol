...
contract PoolManager {
    ...
    function createBettingPool(
        ...
        address[] memory _whitelist) external returns (address) {
        ...
        BettingPool pool = new BettingPool(
            msg.sender, _title, _description,  
             _gameId, _gameType, _endDate,
            _currency, _poolFee, _depositedCal, _whitelist);
        require(IERC20(oracle.getCalAddress()).transferFrom(msg.sender, address(pool), _depositedCal), "Unable to deposit CAL");
    ...
    function getOwnPools(uint8 _poolType) external view returns (address[] memory) {
    ...
    function getPoolsWithType(uint8 _poolType) external view returns (address[] memory) {
